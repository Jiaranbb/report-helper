...
本文件只处理内部构建稿保存、可选日志追加、PDF 渲染和 EPUB 生成。进入交付阶段后，不再改动核心分析和正文判断；发现事实或结构问题时，回到 `workflow.md` 的审核步骤处理。
...
该脚本会检查 Python 版本、`config.local.json`、报告署名、交付格式、PDF 渲染依赖，以及 Chrome fallback 是否可用。不通过时，按脚本提示补配置或安装依赖。
...
`delivery_formats` 控制公开交付物，支持 `["pdf"]`、`["epub"]`、`["pdf", "epub"]`。如果通过环境变量设置，使用逗号分隔，例如 `REPORT_HELPER_DELIVERY_FORMATS=pdf,epub`。用户在任务里明确指定交付格式时，用户要求优先。

## 2. 保存内部 Markdown 构建稿

Markdown 是内部构建稿，只用于渲染 PDF 和生成 EPUB。构建稿直接从正文 H1 开始。
...
- 交付物：`./output/{研究对象}深度研究报告.pdf`、`./output/{研究对象}深度研究报告.epub`
- {关键发现}
```
...
python scripts/append_report_log.py \
  ...
  --bullet "交付物：\`./output/{研究对象}深度研究报告.pdf\`、\`./output/{研究对象}深度研究报告.epub\`" \
  --bullet "{key finding}"
```

## 4. 按需渲染 PDF

当交付格式包含 PDF 时，使用 fallback 渲染脚本：
...
当交付格式包含 EPUB 时，使用 EPUB 生成脚本：

```bash
python scripts/md_to_epub.py \
  ./output/work/{file-name}.md \
  ./output/{研究对象}深度研究报告.epub \
  --title "{H1 title}" \
  --author "{配置的报告署名}"
```

EPUB 是公开交付物之一。交付时提示用户：EPUB 文件可以导入微信读书打开阅读。

PDF 和 EPUB 最末尾必须追加固定工具签名：
...
- 如果交付 PDF：PDF 文件存在且大小不为 0
- 如果交付 PDF：能读取页数
- 如果交付 PDF：封面标题正确
- 如果交付 PDF：封面作者为配置的报告署名
- 如果交付 PDF：首页视觉上可读
- 如果交付 EPUB：EPUB 文件存在且大小不为 0
- 如果交付 EPUB：EPUB 元数据作者为配置的报告署名
- PDF / EPUB 最末尾包含固定工具签名
