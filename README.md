...
它的目标不是给出几段概述，而是完成一套更接近研究写作的流程：先明确研究范围，再搜集和核查资料，整理证据链，形成核心判断，最后生成一份可直接分享的 PDF 文档。

## 这个 Skill 的特点

- **一句话启动，自动跑完整报告**：给出研究对象后，skill 会自动完成范围确认、资料搜集、写作、审核和 PDF 生成，目标是一篇高质量的长篇深度研究报告。
- **资料搜集扎实，强调可追溯**：报告会保留来源等级和编号，正文关键事实判断对应到具体出处，读者可以回到文末来源列表核查。
- **交付物是正式 PDF**：最终输出是排版后的 PDF 文档，背景色、字号、间距、标题层级、表格和引用块都做过适配，可以直接分享给读者。

## 适合用来做什么

- 产品、公司、人物、概念的深度研究
- 产业链、政策、趋势、年度展望类研究报告
- 需要较长篇幅、可追溯来源和 PDF 成品的分析文章
