...
- **不是普通摘要工具**：目标是生成 1-3 万字级别的长篇研究报告，而不是几段式概述。
- **先审题再写作**：会先对齐研究对象、研究边界、报告体裁、关注重点和交付格式，避免一上来就跑偏。
- **分阶段自检**：资料搜集后会做信息充分性审计，写作前会做判断质量和证据链审计。
- **强调来源可追溯**：正文关键事实判断使用 `a1`、`b4` 这类编号标注来源，文末按来源等级集中列出。
- **区分资料和判断**：不只堆资料，会要求从时间线、竞争格局和未来变量里形成明确判断。
- **适合正式交付**：默认生成 PDF 文档和 EPUB 电子书，EPUB 可导入微信读书阅读。
- **支持本地配置**：输出目录、报告署名、交付格式、日志路径等都可以通过本地配置或环境变量调整。
- **保留中间资料**：会保存中间研究材料和审计结果，方便回看事实来源、缺口和推理过程。
